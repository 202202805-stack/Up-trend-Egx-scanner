"""
================================================================================
ADAM & EVE BACKTESTER & SCANNER (1-YEAR DATA + TRADINGVIEW + OPEN POSITIONS)
================================================================================
"""
...
def get_combined_stock_data(ticker: str, start_date: datetime, end_date: datetime, tv_live_dict: dict) -> pd.DataFrame:
    ...
    if ticker in tv_live_dict:
        ...
        # If last date in YFinance is older than today, append TV live bar
        if df.index[-1].date() < today_date.date():
            ...
            # Update the latest available bar with TradingView close/high/low/volume
...
class AdamEveDetector:
    ...
    def detect(self, df: pd.DataFrame) -> List[AdamEvePattern]:
        df = self._prepare_dataframe(df)
        bottoms = self._find_local_minima(df)
        classified = self._classify_bottoms(df, bottoms)

        adams = [b for b in classified if b.bottom_type == BottomType.ADAM]
        eves = [b for b in classified if b.bottom_type == BottomType.EVE]

        patterns = self._match_adam_eve(df, adams, eves)
    ...
    def _match_adam_eve(self, df: pd.DataFrame, adams: List[Bottom], eves: List[Bottom]) -> List[AdamEvePattern]:
        patterns = []
        highs = df['High'].values
    ...
    def _check_preceding_trend(self, df: pd.DataFrame, adam_idx: int) -> bool:
        if adam_idx < self.min_trend_bars:
            return False
        start_idx = adam_idx - self.min_trend_bars
        trend_high = df['High'].iloc[start_idx:adam_idx].max()
        adam_low = df['Low'].iloc[adam_idx]
        decline_pct = ((trend_high - adam_low) / trend_high) * 100
        return decline_pct >= self.min_trend_decline_pct

    def _check_breakouts(self, df: pd.DataFrame, patterns: List[AdamEvePattern]) -> List[AdamEvePattern]:
        closes = df['Close'].values
        volumes = df['Volume'].values
        avg_volume = volumes.mean()

        for p in patterns:
            eve_idx = p.second_bottom.index
            max_check = min(len(df), eve_idx + self.max_bars_after_second_bottom + 1)
            required_breakout_price = p.peak * (1 + (self.breakout_buffer_pct / 100.0))

            breakout_found = False
            for i in range(eve_idx + 1, max_check):
    ...
    def _score_patterns(self, patterns: List[AdamEvePattern]) -> List[AdamEvePattern]:
        for p in patterns:
            score = 50.0
            ...
            if p.first_bottom.volume_ratio > p.second_bottom.volume_ratio:
                score += 10
    ...
    def _enforce_exclusivity(self, patterns: List[AdamEvePattern]) -> List[AdamEvePattern]:
        if not patterns:
            return []
...
# ====================================================================
# 4. BACKTEST ENGINE & OPEN POSITIONS SUPPORT
# ====================================================================

class Backtester:

    def __init__(self, tickers: List[str], years: int = 1):
        self.tickers = tickers
        self.years = years
        self.detector = AdamEveDetector()
        self.trades = []
    ...
    def run(self):
        ...
        print(
            f"Starting Backtest & Pattern Scan from {start_date.strftime('%Y-%m-%d')} to"
    ...
    def _simulate_trade(self, df: pd.DataFrame, ticker: str, p: AdamEvePattern):
        ...
        # If exit_date is None -> Trade is still OPEN -> Exit Date remains empty ("")
    ...
    def export_to_excel(self, filename: str = 'EGX_Adam_Eve_Matched_Format.xlsx'):
        if not self.trades:
            print('\nNo trades were generated to export.')
            return

        trades_df = pd.DataFrame(self.trades)
        
        # Deduplicate trades by Stock Name and Entry Date
        ...
        # Updated Header Layout (8 Columns)
        columns = [
            'Stock Name',
            'Entry Date',
            'Entry Price',
        ...
        try:
            ...
            print(f'Excel File Saved with Exact Required Format: {filename}')
            print('========================================================')
        except PermissionError:
            alt_filename = f"EGX_Adam_Eve_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            wb.save(alt_filename)
            print(f"\n[Permission Error] Main file was locked. Saved as: {alt_filename}")
...
if __name__ == '__main__':
    # Runs scan on 1-year window
    backtester = Backtester(tickers=EGYPTIAN_STOCKS, years=1)
    backtester.run()
