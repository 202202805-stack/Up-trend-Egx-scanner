"""
================================================================================
EVE & ADAM BACKTESTER & SCANNER (1-YEAR DATA + TRADINGVIEW + OPEN POSITIONS)
================================================================================
"""
...
@dataclass
class EveAdamPattern:
    first_bottom: Bottom   # Eve (القاع المدور أولاً)
    second_bottom: Bottom  # Adam (القاع الحاد ثانياً)
    peak: float
    peak_index: int
...
class EveAdamDetector:
    ...
    def detect(self, df: pd.DataFrame) -> List[EveAdamPattern]:
        ...
        patterns = self._match_eve_adam(df, adams, eves)
    ...
    def _match_eve_adam(self, df: pd.DataFrame, adams: List[Bottom], eves: List[Bottom]) -> List[EveAdamPattern]:
        """Matches Eve (first) with Adam (second) bottoms."""
        patterns = []
        highs = df['High'].values

        for first in eves:  # Eve يجي الأول
            for second in adams:  # Adam يجي الثاني
    ...
    def _check_breakouts(self, df: pd.DataFrame, patterns: List[EveAdamPattern]) -> List[EveAdamPattern]:
        closes = df['Close'].values
        volumes = df['Volume'].values
        avg_volume = volumes.mean()

        for p in patterns:
            adam_idx = p.second_bottom.index
            max_check = min(len(df), adam_idx + self.max_bars_after_second_bottom + 1)
            required_breakout_price = p.peak * (1 + (self.breakout_buffer_pct / 100.0))

            breakout_found = False
            for i in range(adam_idx + 1, max_check):
    ...
    def _score_patterns(self, patterns: List[EveAdamPattern]) -> List[EveAdamPattern]:
        for p in patterns:
            score = 50.0
            ...
            # في نموذج Eve & Adam يفضل عادةً أن يكون حجم التداول عند Adam أعلى
    ...
    def _enforce_exclusivity(self, patterns: List[EveAdamPattern]) -> List[EveAdamPattern]:
        if not patterns:
            return []
...
class Backtester:
    ...
    def _simulate_trade(self, df: pd.DataFrame, ticker: str, p: EveAdamPattern):
    ...
    def export_to_excel(self, filename: str = 'EGX_Eve_Adam_Matched_Format.xlsx'):
        if not self.trades:
        ...
        try:
            ...
            alt_filename = f"EGX_Eve_Adam_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
if __name__ == '__main__':
    # Runs scan on 1-year window for Eve & Adam pattern
